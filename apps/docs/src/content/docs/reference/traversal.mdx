...
Builds a graph from an actor definition. Nodes are states, edges are
transitions. Effect self-loops and region children are included.

`internalIds` marks event types as internal. Their edges are excluded from
coverage. `sampleContexts` provides context values for handler evaluation.
...
Recursively walks a snapshot tree and adds active state node IDs to the set.
Used internally by `buildGraph` to mark which nodes are active.
...
Wraps an actor to record every state visit, transition, and effect in a
`History` instance. Returns an `InstrumentedActor` with the same API plus a
`history` property.
...
Append-only log of actor activity. Query by type or get derived sets.
...
BFS reachability. Can `fromId` reach `toId`?
...
Returns all cycles in the graph as arrays of node IDs.
