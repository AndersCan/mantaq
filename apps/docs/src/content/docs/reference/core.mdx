...
| `.create(payload)`  | `{ state, payload }`         | Make state entry   |
...
| Property  | Type                       | What            |
| --------- | -------------------------- | --------------- |
| `state`   | `States[number]`           | Current state   |
...
The type-safe registrar passed to `setup`. Targets validate against declared states, inputs, and internal events — an undeclared target fails to compile.

```ts
setup: (m) => {
  m.on(idle, start, () => ({ state: running }));
  m.onAny(cancel, () => ({ state: idle }));
