...
| `.create()`        | `{ id: T }`           | Void-payload instance      |
| `.create(payload)` | `Payload & { id: T }` | Payload instance           |
...
| `CreatedOf<E>`             | Event instance type for `E`: `{ id }` or `Payload & { id }` |
| `CreatedOfEvent<Id, P>`    | Same, explicit id and payload                               |
| `EffectFn<C, P>`           | `(input: EffectInput<C, P>) => void`                        |
