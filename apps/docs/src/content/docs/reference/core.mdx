...
| `.regions(options)` | `this`                       | Declared but not yet wired (currently a no-op; kept internal) |
...
| `inject(event)`        | `(event: InternalEvent) => void`             | Push an internal event and drain (tests/embedders)          |
| `dispose()`            | `() => void`                                 | Stop: abort effect, clear queue + subscribers               |
| `recover(target)`      | `(target: { state; context }) => void`       | Restart a dead machine from caller-supplied state           |
...
Effects run on every state entry. `name` is a required camelCase string
describing what the effect does — executed effects are recorded in history as
`{ stateName, effectName }`, and tests can assert on them by name
(`harness.assertEffectRan(stateName, effectName)`). The initial state runs its
effects at construction with the synthetic event `{ type: "__init" }`; terminal
states run theirs on entry (useful for emitting a completion output to a
parent).
