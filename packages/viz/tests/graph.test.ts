...
function createSimpleActor() {
  ...
  return new Actor({
    ...
    context: {} as {},
    setup: (m) => {
      m.on(idle, fetch, () => ({ state: loading }));
      m.on(loading, success, () => ({ state: done }));
    },
  });
}
...
describe("buildGraph with effect timers", () => {
  test("pendingTimers exposes eventName from clock", () => {
    ...
    const actor = new Actor({
      ...
      clock,
      setup: (m) => {
        m.on(idle, start, () => ({ state: working }));
        m.on(working, timeout, () => ({ state: idle }));
        m.effect(working, ({ signal, clock: c }) => {
