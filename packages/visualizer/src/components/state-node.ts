const stateNodeStyles = `
  ...
  .node { cursor: pointer; transition: filter 0.2s ease; }
  ...
  .node-label {
    ...
    font-size: 13px; fill: var(--viz-node-label); pointer-events: none; user-select: none;
  }
  ...
  .active-glow {
    ...
    stroke-width: 3; opacity: 0.4; rx: 8; animation: glow-pulse 2s ease-in-out infinite;
  }
  ...
  @media (prefers-reduced-motion: reduce) { .active-glow, .selection-ring { animation: none; } }
`;
...
function renderStateNode(el: StateNode) {
  ...
  el.innerHTML =
    `<style>${stateNodeStyles}</style>` +
    `<svg width="${width + 20}" height="${height + 20}" role="button" tabindex="0" aria-label="${aria}" ` +
    ...
    `<text class="node-label" x="${width / 2}" y="${height / 2}" text-anchor="middle" dominant-baseline="central">${label}</text>` +
    (isFinal
