...
- **Load worker SKILL.md from disk in prompt.** Include full path to `self-improvement-worker/SKILL.md` in every worker prompt. Workers need explicit instructions. Absolute paths (`/app/mantaq/.opencode/skills/...`) are safer than relative.
- **Pre-discover all tasks before starting.** Single explore agent at start to map all TODOs, bugs, gaps. Build ranked DAG. Workers execute DAG in priority order. Prevents mid-run discovery and re-prioritization.
- **Workers can handle lit-html conversions safely.** Converting `innerHTML` to `lit-html` render with Shadow DOM requires test updates (shadowRoot querying). Include in prompt that tests need `shadowRoot!.querySelector` pattern. All lit-html conversions in one run passed tests on first try.
- **Sequential > parallel for same-branch commits.** 10 sequential workers on same branch (no parallelism) avoids merge conflicts entirely. Each commit builds on previous. Trade-off: slower wall clock but zero conflict resolution. For <15 iterations, sequential is fine. For >15, batch into groups.
- **`vp check && vp test` is reliable gate.** Passed all 10 iterations. No false negatives. Trust this over manual review.
- **Default branch detection.** Skill assumes `wip` branch but real default may be `main`. Check `git remote show origin | grep HEAD` or `git symbolic-ref refs/remotes/origin/HEAD` before creating PR base.
