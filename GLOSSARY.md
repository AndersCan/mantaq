...
| **Entry action**               | Code run every time a state is entered — first entry included.                                             | `m.effect(state, fn)` — runs on entry into that state.                                      |
...
| **Timeout**                    | A transition triggered by elapsed time instead of a message. Time is another event source.                 | `clock.setTimeout(...)` inside an effect.                                                   |
